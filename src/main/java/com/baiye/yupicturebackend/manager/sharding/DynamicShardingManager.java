...
public class DynamicShardingManager {
    ...
    //@PostConstruct注解标记的方法会在 Spring 容器创建并配置完这个 Bean 后立即执行。
    @PostConstruct
    ...
    private Set<String> fetchAllPictureTableNames() {
        ...
        //查询数据库，找出所有 spaceType 为 TEAM 的空间（团队空间），并为每个空间生成一个对应的表名（格式为 picture_{spaceId}）
    ...
    private void updateShardingTableNodes() {
        Set<String> tableNames = fetchAllPictureTableNames();//获取所有动态表名
        //例如 "yu_picture.picture, yu_picture.picture_1, yu_picture.picture_2"
        ...
        if (shardingRule.isPresent()) {
            ...
            List<ShardingTableRuleConfiguration> updatedRules = ruleConfig.getTables()
                    ...
                    .map(oldTableRule -> {
                        if (LOGIC_TABLE_NAME.equals(oldTableRule.getLogicTable())) {
                            ...
                            //保留原有的数据库分片策略、表分片策略、主键生成策略和审计策略
            ...
            ruleConfig.setTables(updatedRules);//用更新后的规则列表替换旧的规则列表。
            //向 ShardingSphere 提交规则变更。
    ...
    public void createSpacePictureTable(Space space) {
        ...
        if (space.getSpaceType() == SpaceTypeEnum.TEAM.getValue() && space.getSpaceLevel() == SpaceLevelEnum.FLAGSHIP.getValue()) {
            ...
            // 创建新表,并使其结构（列定义、索引等）完全复制自一个已存在的表。(picture)
            String createTableSql = "CREATE TABLE " + tableName + " LIKE picture";
            try {
                SqlRunner.db().update(createTableSql);// 执行 SQL
                // 更新分表
                updateShardingTableNodes();
            } catch (Exception e) {
